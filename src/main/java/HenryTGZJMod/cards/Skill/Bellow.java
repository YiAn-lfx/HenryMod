...
public class Bellow extends AbstractHenryCard {
    public static final String ID = ModHelper.makePath(Bellow.class.getSimpleName());
    ...
    public void upgrade() { //卡牌升级
        if(!this.upgraded){
            this.upgradeName();
            this.upgradeBaseCost(0); //提升的伤害数值
    ...
    public void use(AbstractPlayer p, AbstractMonster m) { //卡牌使用效果
        this.addToBot(
                new ApplyPowerAction(
                        p, p, new StancePower(p,magicNumber)
                )
        );
        this.addToBot(
                new ApplyPowerAction(
                        p, p, new StrengthPower(p,magicNumber)
                )
        );
        this.addToBot(
                new ApplyPowerAction(
                        p, p, new LoseStrengthPower(p,magicNumber)
                )
        );
