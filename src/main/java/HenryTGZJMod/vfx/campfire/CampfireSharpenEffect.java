package HenryTGZJMod.vfx.campfire;
...
public class CampfireSharpenEffect extends AbstractGameEffect {
    private static final float DUR = 2.0F;
    private boolean hasRepaired = false;
    private Color screenColor;

    public CampfireSharpenEffect() {
    ...
    public void update() {
        ...
        if (this.duration < 1.0F && !this.hasRepaired) {
            this.hasRepaired = true;
            if (AbstractDungeon.player.hasRelic("HenryTGZJMod:WeaponStatus")) {
                AbstractDungeon.player.getRelic("HenryTGZJMod:WeaponStatus").flash();
                AbstractDungeon.player.getRelic("HenryTGZJMod:WeaponStatus").counter = 120;
